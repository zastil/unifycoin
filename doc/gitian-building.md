...
*Setup instructions for a Gitian build of Unifycoin Core using a Debian VM or physical system.*
...
- [Building Unifycoin Core](#building-unifycoin-core)
...
Clone the git repositories for unifycoin and Gitian.
...
in the unifycoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Unifycoin Core
----------------

To build Unifycoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the unifycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/unifycoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/unifycoin/unifycoin
...
For example:
```bash
URL=https://github.com/thrasher-/unifycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit unifycoin=${COMMIT} --url unifycoin=${URL} ../unifycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit unifycoin=${COMMIT} --url unifycoin=${URL} ../unifycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit unifycoin=${COMMIT} --url unifycoin=${URL} ../unifycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the unifycoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../unifycoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/unifycoin
SIGPATH=/some/root/path/unifycoin-detached-sigs

./bin/gbuild --url unifycoin=${BTCPATH},signature=${SIGPATH} ../unifycoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/unifycoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/unifycoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/unifycoin-osx-build.assert
```
...
[unifycoin-project/gitian.sigs.ltc](https://github.com/unifycoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
