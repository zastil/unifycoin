...
const char * const BITCOIN_CONF_FILENAME = "unifycoin.conf";
const char * const BITCOIN_PID_FILENAME = "unifycoin.pid";
...
#else
    const char* pszModule = "unifycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Unifycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Unifycoin";
#else
    // Unix
    return pathRet / ".unifycoin";
#endif
#endif
}
